...
const DonorRegisterForm = ({ step, totalSteps, nextStep }: IStepComponentProps) => {
  ...
  const handleFormSubmit = async (formData: IRegisterAsDonorRequest) => {
    ...
    try {
      const response = await FakeRequestService.post("/register-as-donor", formData, { successData: formData });
